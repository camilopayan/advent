...
def get_all_matches(line, regex)
  line.to_enum(:scan, regex).map { Regexp.last_match }
end

def line_sum(line)
  numbers = []
  get_all_matches(line, /[^.\d\s](\d+)/).each do |match|
    num = match[1]
    numbers << num.to_s.to_i
    # line.sub!(/#{num}/, '.' * num.to_s.length)
  end
  get_all_matches(line, /(\d+)[^.\d\s]/).each do |match|
    num = match[1]
    numbers << num.to_s.to_i
    # line.sub!(/#{num}/, '.' * num.to_s.length)
  end
...
def symbol_positions(line)
  get_all_matches(line, /([^.\d\s])/).map { |m| m.begin(1) }
end

def adjacents_sum(line, positions)
  matches = get_all_matches(line, /(\d+)/)
